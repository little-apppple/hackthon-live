...
description: 基于《Vibe Coding 开发流程与体系》的 AI 辅助开发全流程 skill，把需求澄清→计划拆解→TDD 实现→Loop Review→E2E 回归→部署上线→复盘沉淀组织成带硬门禁的工作循环，且七个卡点与黑客松 hackathon-reporter 的七节点上报一一对应。Use when 新项目启动/按 VibeCoding 流程开发/参赛项目需要按比赛卡点推进/需要生成 CLAUDE.md、AGENTS.md 或自动检测安装 skill 依赖时。
---
...
工作循环的每一步既是 VibeCoding 生命周期阶段，也是 report 的上报卡点：

| # | VibeCoding 阶段 | 卡点 | 机器可判定验收标准 | 上报动作 |
|---|---|---|---|---|
| 1 | 需求澄清 → 规格沉淀 | `requirements` | spec/需求清单落盘 docs/，圈定演示主线 | `--stage requirements` |
| 2 | 计划拆解 → 方案定稿 | `design` | 技术选型/架构/核心接口契约定稿，任务清单人工过目 | `--stage design` |
| 3 | 原型确认 | `prototype` | 页面/交互原型确认，E2E 要覆盖的核心旅程清单（5–10 条）定稿 | `--stage prototype` |
...
node report.js --stage <卡点> / --deploy / --verify
        ↓
node report.js --next   ← 循环，直到「全部节点已完成」
```

非参赛项目没有服务端时，同样按七卡点顺序推进，只是不执行上报命令；每张表的「验收标准」仍是硬门禁。
