...
description: 黑客松参赛项目进度自动上报与线上验收。当参赛项目完成需求分析、方案设计、原型设计、代码开发、本地测试、上线部署中任一节点时，调用 report.js 向赛事服务端汇报进度；部署完成后通过 --verify 自动执行探活/接口测试/E2E 并上报验收。Use when 黑客松参赛项目完成任一阶段需要上报进度、或需要查询当前上报状态、或部署后需要线上验收时。
---
...
| 1 / 网络 | `NETWORK_ERROR` | 服务端不可达 | 稍后重试一次；仍失败则提示用户检查网络/服务端 |
