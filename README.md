...
| 口径 | 说明 |
|---|---|
| **进度加权** | 节点权重不同：需求/设计/原型各 5%、代码开发 30%、本地测试 15%、上线部署/线上验收各 15%、最终提交 10%——避免「写完 PRD 就 25%」的观感误差（`server/src/stages.js` 的 `weight`） |
| **自报 vs 服务端验证** | 1-5 节点为小组自报（圆点画**空心环**）；6-8 节点由服务端机器验证（部署探活、验收三关、提交确认，圆点**实心带微光**）——评委一眼看出哪些结论经过机器验证 |
| **停滞与活跃** | 卡片显示当前节点已耗时（`stageStartedAt`）；CLI 每次 `--next/--status/上报` 续期 `last_seen_at`，停滞 ≥30 分钟标注「停滞中」，有活动则显示「最近活动 X 分钟前」（自动派生，无需队伍手工维护） |
| **可体验标签** | 项目上线（第 6 节点）后卡片显示「可体验」，引导评委点开 |
| **交付形态提前标注** | `--init --deliverable package`（默认 `web`）在注册时即告知类型；安装包类项目的「原型设计」节点在大屏标为不涉及（虚线环），节点口径不变 |
| **观众向文案** | 节点保留开发名称，卡片同时展示通俗说法（上线部署=能玩了、线上验收=机器验过、最终提交=定稿提交），悬停显示数据来源 |
| **KPI 主指标** | 「已上线」「已提交」为主（带占比），「验收通过」降为副文案，避免三档口径混淆 |
...
**注册信息（大屏/后台展示用）**：自助注册可一并提交 `summary`（一句话需求，展示用、不是流程里的需求分析）、`members`（参与人员，「人员维度」页数据源）、`value`（价值）、`features`（功能）、`scenario`（场景）；CLI 交互注册逐项提示（可跳过），也支持同名命令行参数一次性带参；**同一客户端重注册可更新这些信息**。大屏点击项目卡/已提交榜项弹出详情，管理端项目表「查看」读同一份信息。
