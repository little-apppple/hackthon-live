...
**幂等语义（核心设计）**：同一期活动内，相同「部门 + 小组 + 项目名称」只生成一次 accessKey；重复执行 `--init` 返回**同一把密钥**，可安全重跑、可多处补发配置。同组不同项目名视为不同项目，各有独立密钥。
...
| **B · 后台建项目发 key**（原有流程） | 后台「项目与密钥」手工建项目 → 复制配置 JSON 发给该队 | 名单已确定、想逐队审核、补录漏军队伍 |
...
| 换了台电脑重新接入 | 同样返回原密钥，配置重新生成 |
| 误把项目名写错、注册了个新项目 | 错误项目请找管理员归档删除；正确名称重新 `--init --force` |
| 同一个小组做两个项目 | 项目名不同即各自独立密钥，互不影响 |
